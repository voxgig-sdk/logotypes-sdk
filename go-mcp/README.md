...
[MCP](https://modelcontextprotocol.io) server exposing the Logotypes SDK as
two agent tools — `logotypes_list` and `logotypes_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/logotypes-mcp)
make build

# 2. Provide credentials via the environment
export LOGOTYPES_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user logotypes \
  -- /absolute/path/to/logotypes-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./logotypes-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// logotypes_list: first page of records
{ "entity": "all" }
{ "entity": "all", "query": { } }

// logotypes_load: one record by id
{ "entity": "get_logo_by_name", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/logotypes-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user logotypes \
     -- "$PWD"/dist/*/logotypes-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `logotypes_list` and `logotypes_load` tools now appear
   in new sessions. Ask the agent to *"list all using logotypes"*
   and it calls `logotypes_list` with `{"entity":"all"}`.
...
```sh
export LOGOTYPES_APIKEY=sk_live_xxx            # API key
export LOGOTYPES_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `logotypes_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `logotypes_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `LOGOTYPES_APIKEY` | API key sent with every request. |
| `LOGOTYPES_BASE` | Optional override of the API base URL. |

### Entities

The 4 entities valid as the `entity` argument:

all | data | get_logo_by_name | logo

### Smoke test via HTTP (raw JSON-RPC)

```sh
./logotypes-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"logotypes_load","arguments":{"entity":"get_logo_by_name","query":{"id":1}}}}'
```
