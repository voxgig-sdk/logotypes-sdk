...
The Ruby SDK for the Logotypes API — an entity-oriented client using idiomatic Ruby conventions.
...
```ruby
client = LogotypesSDK.test(testopts, sdkopts)
```
...
raise a `LogotypesError` (a `StandardError` subclass), so wrap
...
```ruby
# load returns the ENTITY — call data_get for the GetLogoByName record (raises on error).
...
```
rb/
├── Logotypes_sdk.rb       -- Main SDK module
...
The main module (`Logotypes_sdk`) exports the SDK class
