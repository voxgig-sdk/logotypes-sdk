...
class AllEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.All(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.All(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set LOGOTYPES_TEST_ALL_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def all_basic_setup(extra)
  ...
  entid_env_raw = ENV["LOGOTYPES_TEST_ALL_ENTID"]
