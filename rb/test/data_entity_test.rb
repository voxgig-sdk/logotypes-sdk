...
class DataEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = LogotypesSDK.test(seed, nil)
    seen = base.Data(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Data(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set LOGOTYPES_TEST_DATA_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def data_basic_setup(extra)
  ...
  idmap = Vs.transform(
    ["data01", "data02", "data03", "logo_name01"],
    {
      "`$PACK`" => ["", {
        "`$KEY`" => "`$COPY`",
  ...
  entid_env_raw = ENV["LOGOTYPES_TEST_DATA_ENTID"]
