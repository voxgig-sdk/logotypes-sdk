...
require_relative 'Logotypes_types'
...
class LogotypesSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = LogotypesHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = LogotypesHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, LogotypesFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    LogotypesUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = LogotypesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = LogotypesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = LogotypesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = LogotypesHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = LogotypesError.new(
        "graphql_error", "LogotypesSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
