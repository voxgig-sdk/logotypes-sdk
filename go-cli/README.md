...
boru-driven command-line client **and** interactive REPL for the Logotypes
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/logotypes-cli)
make build
...
./logotypes-cli list all
./logotypes-cli list data

# 5. Override the API base URL for a single call
LOGOTYPES_BASE=https://api.example.com ./logotypes-cli list all
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/logotypes-cli
   ```
...
Run with no arguments to open a REPL (prompt `logotypes>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/logotypes-cli`. |
...
(`./logotypes-cli <expr>`) and interactively (the REPL), and expressions compose the
