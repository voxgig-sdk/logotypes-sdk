// Command Logotypes-cli is a CLI + REPL for the Logotypes SDK,
...
const prompt = "logotypes"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "all data get_logo_by_name logo"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      LOGOTYPES_APIKEY (api key), LOGOTYPES_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: LOGOTYPES_APIKEY carries the API key and
	// LOGOTYPES_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("LOGOTYPES_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("LOGOTYPES_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewLogotypesSDK(opts)
