...
describe('GetLogoByNameDirect', async () => {
  ...
  test('direct-load-get_logo_by_name', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-get_logo_by_name', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['LOGOTYPES_TEST_GET_LOGO_BY_NAME_ENTID']
