...
//     (\`new LogotypesSDK(...)\` / \`LogotypesSDK.test(...)\` become
//     \`LogotypesSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"all":{"test01":{"id":"test01"}},"data":{"test01":{"id":"test01"}},"get_logo_by_name":{"test01":{"id":"test01"}},"logo":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new LogotypesSDK(...)\` and any
// \`LogotypesSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+LogotypesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/LogotypesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
