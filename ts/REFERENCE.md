# Logotypes TypeScript SDK Reference

Complete API reference for the Logotypes TypeScript SDK.
...
```ts
new LogotypesSDK(options?: object)
```
...
#### `LogotypesSDK.test(testopts?, sdkopts?)`
...
**Returns:** `LogotypesSDK` instance in test mode.
...
#### `GetLogoByName(data?: object)`

Create a new `GetLogoByName` entity instance.
...
**Returns:** `GetLogoByNameEntity` instance.
...
Return the parent `LogotypesSDK` instance.
