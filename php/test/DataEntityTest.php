...
class DataEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = LogotypesSDK::test($seed, null);
        $seen = iterator_to_array($base->Data(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = LogotypesConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = LogotypesSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->Data(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set LOGOTYPES_TEST_DATA_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function data_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["data01", "data02", "data03", "logo_name01"] as $k) {
    ...
    $entid_env_raw = getenv("LOGOTYPES_TEST_DATA_ENTID");
