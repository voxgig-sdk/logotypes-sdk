...
class FtCtrl extends LogotypesControl
{
...
class FtClient
{
    ...
    private LogotypesUtility $utility;

    public function __construct(LogotypesUtility $utility, array $options)
    {
    ...
    public function get_utility(): LogotypesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (LogotypesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public LogotypesContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (LogotypesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = LogotypesConfig::make_feature($name);
    ...
    private static function buildUrl(LogotypesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof LogotypesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new LogotypesResponse($fetched);
        }
    ...
    private function fail(LogotypesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (LogotypesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
