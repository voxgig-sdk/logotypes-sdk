...
An open-source API that provides a vast collection of logos for various projects.

Learn more about Logotypes API at [logotypes.dev](https://logotypes.dev).

This is an unofficial SDK for the Logotypes public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — All, Data, GetLogoByName and Logo — that you
...
const items = await client.All().list()
```
...
| **Data** | The Data entity (list). | `/random/data` |
| **GetLogoByName** | The GetLogoByName entity (load). | `/{logoName}` |
| **Logo** | The Logo entity (load). | `/random` |
...
$alls = $client->All()->list();
print_r(array_map(fn($item) => $item->data_get(), $alls));
```
