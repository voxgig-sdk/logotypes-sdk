...
module LogotypesUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = LogotypesResponse.new({})
    ...
    if fetch_err
      ...
      response = LogotypesResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = LogotypesResponse.new(fetched)
    else
