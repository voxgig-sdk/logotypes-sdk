...
// Logotypes SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class LogotypesMakePoint
{
    public static function call(LogotypesContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof LogotypesError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = LogotypesHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = LogotypesHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
