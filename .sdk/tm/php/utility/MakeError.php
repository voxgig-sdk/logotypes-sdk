...
class LogotypesMakeError
{
    public static function call(?LogotypesContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new LogotypesContext([], null);
        }
        $op = $ctx->op ?? new LogotypesOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new LogotypesResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof LogotypesError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new LogotypesError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof LogotypesError) {
