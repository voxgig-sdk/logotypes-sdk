...
class LogotypesRetryFeature extends LogotypesBaseFeature
{
    ...
    public function init(LogotypesContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (LogotypesContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(LogotypesContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(LogotypesContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
