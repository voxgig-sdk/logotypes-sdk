...
class TestDataDirect:
    ...
    def test_should_direct_list_data(self):
        setup = _data_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-data", "live" if setup["live"] else "unit")
        if _skip:
