...
class TestDataEntity:
    ...
    def test_should_stream(self):
        ...
        base = LogotypesSDK.test(seed, None)
        seen = list(base.Data(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Data(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set LOGOTYPES_TEST_DATA_ENTID JSON to run live")
        client = setup["client"]
...
def _data_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("LOGOTYPES_TEST_DATA_ENTID"))
