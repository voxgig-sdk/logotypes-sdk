...
class TestAllDirect:
    ...
    def test_should_direct_list_all(self):
        setup = _all_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-all", "live" if setup["live"] else "unit")
        if _skip:
