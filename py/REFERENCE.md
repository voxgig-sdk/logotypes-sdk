# Logotypes Python SDK Reference

Complete API reference for the Logotypes Python SDK.
...
#### `LogotypesSDK.test(testopts=None, sdkopts=None)`
...
#### `GetLogoByName(data=None)`

Create a new `GetLogoByNameEntity` instance. Pass `None` for no initial data.
...
```python
result, err = client.GetLogoByName().load({"id": "get_logo_by_name_id"})
```
...
Create a new `GetLogoByNameEntity` instance with the same options.
