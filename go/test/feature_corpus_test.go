...
//
// NewLogotypesSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.LogotypesSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewLogotypesSDK(opts)
}
...
func fcCandidates(client *sdk.LogotypesSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.LogotypesSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.LogotypesSDK, name string) any {
