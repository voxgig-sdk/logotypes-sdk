...
import (
	...
	omni "github.com/voxgig-sdk/logotypes-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.LogotypesError); is {
				out := map[string]any{
					"name":    "LogotypesError",
...
func omniCtx(arg any, client *sdk.LogotypesSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.LogotypesSDK); is {
			client = live
