...
The Lua SDK for the Logotypes API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:All(nil):list(nil, nil)
...
local result, err = client:Logotypes(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── logotypes_sdk.lua    -- Main SDK module
